...
public class SupporterManager {
    ...
    public static Supporter getRandomHunter(@NotNull RandomSource rnd) {
        ...
        return new Supporter(VampirismAttachments.Keys.HUNTER_PLAYER, "none", "none", null, new HashMap<>());
    }
    ...
    public static Supporter getRandomVampire(@NotNull RandomSource rnd) {
        ...
        return new Supporter(VampirismAttachments.Keys.VAMPIRE_PLAYER, "none", "none",  null, new HashMap<>());
    }
    ...
    public static void init() {
        VampirismAPI.settings().getSupportersAsync().thenAccept(optional -> {
            optional.ifPresentOrElse(supporters -> {
                ...
                supporter[0] = supporters.stream().filter(s -> s.faction().equals(VampirismAttachments.Keys.VAMPIRE_PLAYER)).toArray(Supporter[]::new);
                supporter[1] = supporters.stream().filter(s -> s.faction().equals(VampirismAttachments.Keys.HUNTER_PLAYER)).toArray(Supporter[]::new);
